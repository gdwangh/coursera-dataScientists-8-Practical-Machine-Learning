...
ModFit1<-train(classe ~ ., train_ds, method="rf",ntree=100)
```
...
ModFit2<-train(train_ds$classe~.,data=trainPC,method="rf",ntree=100)
```
```{r}
...
Considering Backwards Selection appoarch to omit features manually, i.e. begin with all variable, omit the feature with smallest importance, train the model and check the accuracy. Repeat the process until accuracy decrease to the thresh.  

```{r omit_predictor,cache=TRUE}
#acc_thresh<-0.9900
acc_thresh<-0.60
overall_accuracy<-(confusionMatrix(valid_ds$classe, pred1)$overall)[1]

while (overall_accuracy > acc_thresh) {
   vImp<-varImp(ModFit1)[1]$importance    # feature importance
   # get features except that with min importance
   remain_predictor<-rownames(vImp)[-which(vImp$Overall==min(vImp))] 

   # ModFit1<-randomForest(train_ds$classe ~ ., train_ds[,remain_predictor])
   ModFit1<-train(train_ds$classe~.,train_ds[,remain_predictor],method="rf",ntree=100)
   pred1<-predict(ModFit1, valid_ds) 
   overall_accuracy<-(confusionMatrix(valid_ds$classe, pred1)$overall)[1]
}
...
ModFit3<-train(train_ds$classe ~ ., train_ds[,remain_predictor], method="rf",ntree=100)
